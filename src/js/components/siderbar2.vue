            ...
            <br>
             <table id="left-top-table">
                ...
                <tr>
                    ...
                    <td class="body-2 font-weight-bold color-red link" @click="newWindowfunction('http://gver.rdc.cwb/00Z');">00Z</td>
                    <td class="body-2 font-weight-bold color-red link" @click="newWindowfunction('http://gver.rdc.cwb/12Z');">12Z</td>
                </tr>
                <tr>
                    <td class="body-2 font-weight-bold text-align-center" >G3</td>
                    <td class="body-2 font-weight-bold color-red link" @click="newWindowfunction('http://gver.rdc.cwb/00Z_G3');">00Z</td>
                    <td class="body-2 font-weight-bold color-red link" @click="newWindowfunction('http://gver.rdc.cwb/12Z_G3');">12Z</td>
                </tr>
            </table>
            <br>
            ...
            <div v-if="item_has_data">
                ...
                <div class="title" v-for="(item, index) in forecast_maps" :key="'map' + index">
                    ...
                    <v-select  
                        ...
                        @change="change_component('Select_' + item.key + item_params[item.param])"
                        @input="close_scorecard()"
                        >
                    </v-select>
                </div>
                ...
                <div class="title" v-for="(item, index) in ScoreCards" :key="'score_card' + index">
                    ...
                    <v-select  
                        ...
                        label="請選擇" 
                        v-model="item_params[item.param]"
                        @change="change_component('Select' + item.key + '_' + item_params[item.param])"
                        @input="close_scorecard()">
                    </v-select>
                </div>
            </div>
  ...
  export default {
    ...
    data: function() {
        return {
            ...
            ensemble_tool:[
                {'text': 'Ensemble Varification:', 'key': 'Ensemble', 'param': 'ensemble'},
            ],
    ...
    methods: {
        ...
        change_component: function(component_name) {
            ...
            var major_type = component_name.split('_')[0]; // Select
            let sub_type = component_name.split('_')[1]; // SurfaceD1
            ...
            // component_name = Select_SurfaceD1
            if (component_name.indexOf('Surface') > -1) {
                major_type = 'SelectSurface';
                sub_type = 'D1';
                ...
                this.$store.dispatch('set_current_srcs', [this.$store.getters.settings['General']['path'] +  this.$store.getters.settings['Menu'][major_type][sub_type]['default_src']]);
            }
                ...
                let config_twopicspath = this.$store.getters.settings['Menu'][major_type][sub_type]['default_src']
                var  ensemble_twopic_arr = [];
                this.$store.dispatch('set_tables_array', this.$store.getters.settings['TopMenu'][major_type][sub_type]['parameter']);
                ...
                var default_src;
                for (var i=0; i<this.items[major_type].length; i++) {
                    if (this.items[major_type][i]['value'] == sub_type) {
                        default_src = this.items[major_type][i]['default_src'];
                        break;
                    }
                }
